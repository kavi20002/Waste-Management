...
export default function DriverRequest() {
  ...
  const scheduledRoute = data ? data.filter(route => route.Status === 'Scheduled') : [];
                ...
                <tr key={route._id} className="hover:bg-gray-100">
                  ...
                  <td className="flex flex-col gap-2">
                    <button
                      ...
                      onClick={() => setShowConfirm({ visible: true, type: 'approve', id: route._id })}
                    >
                      Approve
                    </button>
                    <button
                      className="px-4 py-2 mr-3 text-white border-none rounded shadow-md outline-none select-none bg-red hover:bg-red"
                      onClick={() => setShowConfirm({ visible: true, type: 'reject', id: route._id })}
                    >
                      Reject
                    </button>
                  </td>
